...
import ma.rougga.qdata.modal.report.ThTRow;
...
public class ThTTableController {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ThTTableController.class);
    private final AgenceController ac = new AgenceController();

    public boolean addRow(ThTRow row) {
        String sql = "INSERT INTO rougga_tht_table ("
    ...
    public boolean updateRow(ThTRow row) {
        String sql = "UPDATE rougga_tht_table SET "
    ...
    public ThTRow getRowById(String id) {
        String sql = "SELECT * FROM rougga_tht_table WHERE id = ?";
        ThTRow row = null;
    ...
    public boolean batchInsert(List<ThTRow> rows) {
    ...
    public boolean batchUpdate(List<ThTRow> rows) {
        boolean isSuccess = false;
        try {
            Connection con = new CPConnection().getConnection();
            con.setAutoCommit(false); // Disable auto-commit
    ...
    //
    public ThTRow getTotaleRowByAgence(String agenceId, String date1, String date2) {
        ThTRow row = new ThTRow();
        try {
    ...
    public ThTRow getTotaleRow(String date1, String date2, String[] agences) {
        ThTRow row = new ThTRow();
        try {
            // Establish connection
    ...
    public List<ThTRow> getRowsByIdAgenceBetweenDates(String agenceId, String date1, String date2) {
        List<ThTRow> emps = new ArrayList<>();
        try {
            // Establish connection
            Connection con = new CPConnection().getConnection();
            String sql
    ...
    public ThTRow getRowByDate(String date, String id_agence, String serviceId) {
        ThTRow row = null;
        try {
            // Establish connection
            Connection con = new CPConnection().getConnection();
    ...
    public boolean updateAgenceFromJson(String date1, String date2, String agenceId) {
        ...
        Agence a;
        List<ThTRow> rowsToInsert = new ArrayList<>();
        List<ThTRow> rowsToUpdate = new ArrayList<>();
        // validationg data
        ...
        if (a != null) {
            ...
            String url = CfgHandler.prepareTableJsonUrl(a.getHost(), a.getPort(), CfgHandler.API_THT_TABLE_JSON,
                    date1, date2);
            ...
            if (json != null) {
                ...
                for (Object s : result) {
                    try {
                        ...
                        ThTRow row = this.getRowByDate(date2,
                                id_agence, serviceId);
                        if (row != null) {
                            try {
                                ...
                                logger.info("ThTRow id: {} found and updated ", row.getId());
    ...
    public List<Map> getTableAsList(String date1, String date2, String[] agences) {
        ...
        for (Agence a : dbs) {
            List<ThTRow> emps = this.getRowsByIdAgenceBetweenDates(a.getId().toString(), date1, date2);
            if (!emps.isEmpty()) {
    ...
    private boolean doesIdExist(UUID uniqueId) {
        try {
            Connection con = new CPConnection().getConnection();
            String sql = "select 1 from rougga_tht_table where id=? limit 1";
