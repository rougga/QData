...
import ma.rougga.qdata.modal.report.GltRow;
...
public class GltTableController {
    ...
    public boolean addRow(GltRow row) {
    ...
    public boolean updateRow(GltRow row) {
    ...
    public GltRow getRowById(String id) {
        String sql = "SELECT * FROM rougga_glt_table WHERE id = ?";
        GltRow row = null;
    ...
    public GltRow getTotaleRow(String date1, String date2, String[] agences) {
        GltRow row = new GltRow();
        try {
            // Establish connection
    ...
    public List<GltRow> getRowsByIdAgenceBetweenDates(String agence_id, String date1, String date2) {
    ...
    public GltRow getRowByDate(String date, String agenceId, String serviceId) {
        GltRow row = null;
        try {
            // Establish connection
            Connection con = new CPConnection().getConnection();
    ...
    public boolean updateAgenceFromJson(String date1, String date2, String agenceId) {
        ...
        Agence a = new Agence();
        List<GltRow> rowsToInsert = new ArrayList<>();
        List<GltRow> rowsToUpdate = new ArrayList<>();
        // validationg data
            ...
            if (json != null) {
                ...
                for (Object s : result) {
                    try {
                        ...
                        GltRow row = this.getRowByDate(date2,
                                agence_id,
                                serviceId);
                        if (row != null) {
                            try {
    ...
    public List<Map> getTableAsList(String date1, String date2, String[] agences) {
        ...
        for (Agence a : dbs) {
            List<GltRow> emps = this.getRowsByIdAgenceBetweenDates(a.getId().toString(), date1, date2);
            if (!emps.isEmpty()) {
    ...
    private boolean doesIdExist(UUID uniqueId) {
        try {
            Connection con = new CPConnection().getConnection();
            String sql = "select 1 from rougga_glt_table where id=? limit 1";
