...
import ma.rougga.qdata.modal.report.ThTTRow;
...
public class ThTTTableController {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ThTTTableController.class);
    private final AgenceController ac = new AgenceController();

    public boolean addRow(ThTTRow row) {
        String sql = "INSERT INTO rougga_thtt_table ("
    ...
    public boolean updateRow(ThTTRow row) {
        String sql = "UPDATE rougga_thtt_table SET "
    ...
    public ThTTRow getRowById(String id) {
        String sql = "SELECT * FROM rougga_thtt_table WHERE id = ?";
        ThTTRow row = null;
    ...
    public boolean batchInsert(List<ThTTRow> rows) {
    ...
    public boolean batchUpdate(List<ThTTRow> rows) {
        boolean isSuccess = false;
        try {
            Connection con = new CPConnection().getConnection();
            con.setAutoCommit(false); // Disable auto-commit
    ...
    //
    public ThTTRow getTotaleRowByAgence(String agenceId, String date1, String date2) {
        ThTTRow row = new ThTTRow();
        try {
    ...
    public ThTTRow getTotaleRow(String date1, String date2, String[] agences) {
        ThTTRow row = new ThTTRow();
        try {
            // Establish connection
    ...
    public List<ThTTRow> getRowsByIdAgenceBetweenDates(String agenceId, String date1, String date2) {
        List<ThTTRow> emps = new ArrayList<>();
        try {
            // Establish connection
            Connection con = new CPConnection().getConnection();
            String sql
    ...
    public ThTTRow getRowByDate(String date, String id_agence, String serviceId) {
        ThTTRow row = null;
        try {
            // Establish connection
            Connection con = new CPConnection().getConnection();
    ...
    // get ThTT row between 2 dates and insert json result to database for every
    // agency
    ...
    // get ThTT row between 2 dates and insert json result to database for one
    // agency
    public boolean updateAgenceFromJson(String date1, String date2, String agenceId) {
        boolean isDone = false;
        Agence a;
        List<ThTTRow> rowsToInsert = new ArrayList<>();
        List<ThTTRow> rowsToUpdate = new ArrayList<>();
        // validationg data
        ...
        if (a != null) {
            logger.info(" -- Updating {}'s ThTT Table ... ", a.getName());
            String url = CfgHandler.prepareTableJsonUrl(a.getHost(), a.getPort(), CfgHandler.API_THTT_TABLE_JSON,
                    date1, date2);
            ...
            if (json != null) {
                ...
                for (Object s : result) {
                    try {
                        ...
                        ThTTRow row = this.getRowByDate(date2,
                                id_agence, serviceId);
                        if (row != null) {
                            try {
                                ...
                                logger.info("ThTTRow id: {} found and updated ", row.getId());
                ...
                logger.info(" --  ThTT Table for {} is Updated. ", a.getName());
                isDone = true;
            } else {
    ...
    // returns whole ThTT table for report page
    public List<Map> getTableAsList(String date1, String date2, String[] agences) {
        ...
        for (Agence a : dbs) {
            List<ThTTRow> emps = this.getRowsByIdAgenceBetweenDates(a.getId().toString(), date1, date2);
            if (!emps.isEmpty()) {
    ...
    // gets unique UUID after checking in ThTT table
    ...
    // checks if the id exists in ThTT table
    private boolean doesIdExist(UUID uniqueId) {
        try {
            Connection con = new CPConnection().getConnection();
            String sql = "select 1 from rougga_thtt_table where id=? limit 1";
    ...
    public boolean restoreOldRowsByAgenceId(UUID id_agence) {
        ...
        if (oldestDate != null) {
            while (new Date().compareTo(oldestDate) > 0) {
                logger.info("Restoring ThTT table data of {} for date:{}", a.getName(), CfgHandler.getFormatedDateAsString(oldestDate));
            ...
            logger.info("Resored ThTT table data of {}", a.getName());
        } else {
