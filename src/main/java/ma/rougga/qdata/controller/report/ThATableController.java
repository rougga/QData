...
import ma.rougga.qdata.modal.report.ThARow;
...
public class ThATableController {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ThATableController.class);
    private final AgenceController ac = new AgenceController();

    public boolean addRow(ThARow row) {
        String sql = "INSERT INTO rougga_tha_table ("
    ...
    public boolean updateRow(ThARow row) {
        String sql = "UPDATE rougga_tha_table SET "
    ...
    public ThARow getRowById(String id) {
        String sql = "SELECT * FROM rougga_tha_table WHERE id = ?";
        ThARow row = null;
    ...
    public boolean batchInsert(List<ThARow> rows) {
    ...
    public boolean batchUpdate(List<ThARow> rows) {
        boolean isSuccess = false;
        try {
            Connection con = new CPConnection().getConnection();
            con.setAutoCommit(false); // Disable auto-commit
    ...
    //
    public ThARow getTotaleRowByAgence(String agenceId, String date1, String date2) {
        ThARow row = new ThARow();
        try {
    ...
    public ThARow getTotaleRow(String date1, String date2, String[] agences) {
        ThARow row = new ThARow();
        try {
            // Establish connection
    ...
    public List<ThARow> getRowsByIdAgenceBetweenDates(String agenceId, String date1, String date2) {
        List<ThARow> emps = new ArrayList<>();
        try {
            // Establish connection
            Connection con = new CPConnection().getConnection();
            String sql
    ...
    public ThARow getRowByDate(String date, String id_agence, String serviceId) {
        ThARow row = null;
        try {
            // Establish connection
            Connection con = new CPConnection().getConnection();
    ...
    // get THA row between 2 dates and insert json result to database for every
    // agency
    ...
    // get THA row between 2 dates and insert json result to database for one
    // agency
    public boolean updateAgenceFromJson(String date1, String date2, String agenceId) {
        boolean isDone = false;
        Agence a;
        List<ThARow> rowsToInsert = new ArrayList<>();
        List<ThARow> rowsToUpdate = new ArrayList<>();
        // validationg data
        ...
        if (a != null) {
            logger.info(" -- Updating {}'s THA Table ... ", a.getName());
            String url = CfgHandler.prepareTableJsonUrl(a.getHost(), a.getPort(), CfgHandler.API_THA_TABLE_JSON,
                    date1, date2);
            ...
            if (json != null) {
                ...
                for (Object s : result) {
                    try {
                        ...
                        ThARow row = this.getRowByDate(date2,
                                id_agence, serviceId);
                        if (row != null) {
                            try {
                                ...
                                logger.info("ThARow id: {} found and updated ", row.getId());
                ...
                logger.info(" --  THA Table for {} is Updated. ", a.getName());
                isDone = true;
            } else {
    ...
    // returns whole THA table for report page
    public List<Map> getTableAsList(String date1, String date2, String[] agences) {
        ...
        for (Agence a : dbs) {
            List<ThARow> emps = this.getRowsByIdAgenceBetweenDates(a.getId().toString(), date1, date2);
            if (!emps.isEmpty()) {
    ...
    // gets unique UUID after checking in THA table
    ...
    // checks if the id exists in THA table
    private boolean doesIdExist(UUID uniqueId) {
        try {
            Connection con = new CPConnection().getConnection();
            String sql = "select 1 from rougga_tha_table where id=? limit 1";
    ...
    public boolean restoreOldRowsByAgenceId(UUID id_agence) {
        ...
        if (oldestDate != null) {
            while (new Date().compareTo(oldestDate) > 0) {
                logger.info("Restoring THA table data of {} for date:{}", a.getName(), CfgHandler.getFormatedDateAsString(oldestDate));
            ...
            logger.info("Resored THA table data of {}", a.getName());
        } else {
